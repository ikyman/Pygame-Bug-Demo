...
class RandomMosaic():
 ...
 def draw(self, parent_surface,  map_center_x, map_center_y, offset_x, offset_y):
  ...
  mosaic_surface = pygame.Surface( (mosaic_width, mosaic_height))
  ...
  for rindex in range(0, mosaic_height):
   for cindex in range(0, mosaic_width):
    pixelated_mosaic[(self.scale_factor*cindex):(self.scale_factor*(cindex+1)), (rindex*self.scale_factor):(rindex+1)*self.scale_factor] = pygame.Color("white")
    
  pixelated_mosaic.close()
